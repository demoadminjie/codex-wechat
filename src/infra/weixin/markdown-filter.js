/**
 * Streaming markdown filter adapted from upstream openclaw-weixin.
 *
 * It keeps common Markdown constructs that render acceptably in Weixin text
 * while stripping syntax that usually harms readability, especially inline
 * images and some emphasis markers around CJK text.
 */
class StreamingMarkdownFilter {
    ...
    switch (this.inl.type) {
      ...
      case "italic": {
        for (let j = 0; j < this.inl.acc.length; j += 1) {
          if (this.inl.acc[j] === "\n") {
            const restored = `*${this.inl.acc.slice(0, j + 1)}`;
      ...
      case "uitalic": {
        for (let j = 0; j < this.inl.acc.length; j += 1) {
          if (this.inl.acc[j] === "\n") {
            const restored = `_${this.inl.acc.slice(0, j + 1)}`;
            this.buf = this.inl.acc.slice(j + 1);
            this.inl = null;
            this.sol = true;
            return restored;
      ...
      case "image": {
        ...
        if (cb === -1 || cb + 1 >= this.inl.acc.length) {
          return "";
        }
        if (this.inl.acc[cb + 1] !== "(") {
          const restored = `![${this.inl.acc.slice(0, cb + 1)}`;
  ...
  static containsCJK(text) {
    return /[\u2E80-\u9FFF\uAC00-\uD7AF\uF900-\uFAFF]/.test(String(text || ""));
  }
}

function filterWeixinOutboundMarkdown(text) {
